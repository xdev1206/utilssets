...
vim_env()
{
  ...
  #download_plugins
  # Source platform-specific vim_ycm script (lowercase OS_TYPE for directory)
  OS_TYPE_LOWER=$(echo "$OS_TYPE" | tr '[:upper:]' '[:lower:]')
  PLATFORM_SCRIPT="$SCRIPT_PATH/../${OS_TYPE_LOWER}/vim_ycm_${OS_TYPE}.sh"
  
  if [ -f "$PLATFORM_SCRIPT" ]; then
    source "$PLATFORM_SCRIPT"
  else
    echo "Warning: Platform-specific vim_ycm script not found: $PLATFORM_SCRIPT"
    echo "Skipping platform-specific vim compilation (only available on Linux)"
  fi
